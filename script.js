const pages = [
  ...
  },
  {
    type: "text",
    lines: [
      "這種怪事你也沒有少聽過，當初並不特別在意，更何況只是晚餐時隔壁桌的話題，酒過三巡後他們的音量不自覺地大了一些，你不需要側耳，就聽得一清二楚，也算是不錯的飯後消遣。"
    ]
  },
  {
    type: "text",
...
unlockBtn.addEventListener("click", async () => {
  ...
  if (!data.ok) {
    failed++;
    lock.querySelector(".lock-panel").classList.add("shake");
...
function makeLine(page){
  const item = page.lines[lineIndex++];
  const el = document.createElement("div");

  if (page.type === "chat") {
    el.className = `chat-line ${item[0]}`;
    content.appendChild(el);
    return { element: el, text: item[1] };
  }
